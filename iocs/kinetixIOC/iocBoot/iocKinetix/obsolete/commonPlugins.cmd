...
# Load an NDFile database.  FYI - This is not supported for the simDetector which does not write files.
...
# Optional: Load EDFPlugin (Keenan Lang Raw File plugin)

#NDFileEDFConfigure("EDF1", 20, 0, "$(PORT)", 0, 0, 0)
#dbLoadRecords("$(ADEDFPLUGIN)/db/NDFileEDF.template", "P=$(PREFIX), R=EDF1:, PORT=EDF1, ADDR=0, TIMEOUT=1, NDARRAY_PORT=$(PORT), NDARRAY_ADDR=0")
#set_requestfile_path("$(ADEDFPLUGIN)/db")
...
dbLoadRecords("$(TOP)/db/alive.db", "P=$(PREFIX),RHOST=164.54.100.11")
